...
geemap$Map$add_time_slider(ee_object = s2, vis_params = visParams, region = tf, )

## Filter image collection

s2b <- s2$filterBounds(tf)
...
s2_pre <- s2_pre$maskClouds()

## vegetation indices

S2 <- s2_pre$scaleAndOffset()$spectralIndices(c('NDVI', 'NDSI', 'EVI'))
...
```{r}
f1 = ## Point and buffer
point <- ee$Feature(ee$Geometry$Point(lon, lat)$buffer(100))
tf <- ee$Feature(ee$Geometry$Point(lon, lat)$buffer(1000))
fc = ee$FeatureCollection(c(point,tf))

S2 = ee$ImageCollection('COPERNICUS/S2_SR')$filterBounds(fc)$filterDate(start_date,end_date)$maskClouds()$scaleAndOffset()$spectralIndices(c('EVI','NDVI'))

S2$size()$getInfo()

# By Region
  ts = S2$getTimeSeriesByRegion(reducer = c(ee$Reducer$median()),
                                geometry = fc,
                                bands = list('EVI','NDVI'),
                                scale = 10)
ts$size()$getInfo()y

cat(unlist(ts$limit(2)$getInfo()))

visParams <- list(bands = c("EVI", "NDVI"), min = -1, max = 1, palette = viridis::cividis(10))


Map$addLayer(ts$first(), visParams = visParams)
