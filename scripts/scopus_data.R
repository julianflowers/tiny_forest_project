library(needs)
needs(rscopus, jsonlite, tidyverse, devtools, umap)

rscopus::get_api_key()

string <- "KEY(\"urban agriculture biodiversity\") AND PUBYEAR > 2005 AND PUBYEAR < 2024"

string <- searchByString(myQuery, key = "8f82f98c6bca4cd26772e164d462a6ae")

## version 0.4
searchByString <- function(string, content = "complete", myStart = 0, retCount = 25, retMax = Inf, mySort = "-coverDate", cursor = "*", outfile, key = key) {
  if (!content %in% c("complete", "standard")) {
    ...
    ##library(httr)
    ##library(XML)
    key <- "8f82f98c6bca4cd26772e164d462a6ae"
    print("Retrieving records.")
    theURL <- httr::GET("https://api.elsevier.com/content/search/scopus", query = list(apiKey = key, query = string, sort = "-coverDate", httpAccept = "application/xml", view = "complete", count = 25, start = 0, cursor = "*")) ## format the URL to be sent to the API
...
searchByID <- function(theIDs, idtype, datatype = "application/xml", content = "complete", myStart = 0, retCount = 25, outfile, key = key) {
  ##library(httr)
  ##library(XML)
  key <- "yourAPIKey"
  if (length(theIDs) == 1) {
...
source_url("https://github.com/christopherBelter/scopusAPI/blob/master/scopusAPI.R?raw=TRUE")


myQuery <- "KEY(\"urban AND biodiversity AND agriculture\") AND PUBYEAR > 2005 AND PUBYEAR < 2023"
...
areas <- subject_areas()[12]

query <- "Urban AND agriculture AND biodiversity"

scopus_search(query, view = "COMPLETE")

httr::GET("https://api.elsevier.com/content/search/scopus?apiKey=8f82f98c6bca4cd26772e164d462a6ae&query=KEY%28%22urban%20agriculture%20biodiversity%22%29%20AND%20PUBYEAR%20%3E%202005%20AND%20PUBYEAR%20%3C%202024&sort=-coverDate&httpAccept=application%2Fxml&view=complete&count=25&start=0&cursor=%2A")


get_scopus_abstracts <- function(query = "Urban AND agriculture AND biodiversity AND review", start = 0){
...
reps <- seq(0, 1000, by = 25)

reps[1:2]

tictoc::tic()
refs_500 <- map_dfr(reps, ~(get_scopus_abstracts(start = .x)))
tictoc::toc()
...
corp <- myScrapers::create_abstract_corpus(df=refs_500 |>
                                             filter(!is.na(doi)) |>
                                     mutate(absText = paste(title, value),
                                            pmid = doi))

corpus <- corp$corpus |>
  filter(!str_detect(word, "\\d+"),
         !str_detect(word, "api"),
         !str_detect(word, "akhdar"))

corpus |>
  arrange(-n)


clust <- create_abstract_cluster(corpus, minPts = 10)

corpus <- corpus %>% filter(!is.na(pmid))
pmid <- pull(corpus, "pmid") %>% unique() %>% enframe()
tsne <- corpus %>% cast_sparse(pmid, word, tf_idf) %>% as.matrix() %>%
  umap()
plot <- tsne$layout %>% data.frame() %>% ggplot(aes(X1, X2)) +
  geom_point() + theme_void()
set.seed(42)
dbscan <- hdbscan(tsne$layout, minPts = 7)
hc_plot <- dbscan$hc %>% plot()
clustering <- data.frame(cbind(pmid = pmid, tsne$layout, cluster = dbscan$cluster)) %>%
  mutate(V2 = as.numeric(as.character(X1)), V3 = as.numeric(as.character(X2)))
clustering <- clustering %>% mutate(clustered = ifelse(cluster ==
                                                         0, "not-clustered", "clustered"))
cluster_count <- length(unique(clustering$cluster))
cluster_size <- clustering %>% count(cluster)
toc()
return <- list(tsne = tsne, plot = plot, dbscan = dbscan,
               corpus = corpus, clustering = clustering, cluster_count = cluster_count,
               cluster_size = cluster_size, hc_plot = hc_plot)
}
...
labels <- create_cluster_labels(corpus, clustering = clust$clustering, top_n = 4)
